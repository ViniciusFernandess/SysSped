...
namespace SysSped.Infra.Data
{
    public class LogRepository : BaseRepository, ILogRepository
    {
        ...
        public void RegistrarLog(Bloco0000 bloco0000, EnumTipoSped tipoBloco, int indiceCampo, string nomeCampo, int indiceLinha, string codigoInterno, string ean, string campoAntigo, string campoNovo)
        {
            var sql = $@"
                        INSERT INTO 
                        `sysspeddb`.`logalteracaosped` 
                        (idbloco0000, nrolinha, indiceCampo, nomeCampo, codigoInterno, ean, tiposped, valorantigo, valoratual, datacadastro, ativo) 
                        VALUES 
                        ({bloco0000.Id}, {indiceLinha + 1}, {indiceCampo}, '{nomeCampo}', '{codigoInterno}', '{ean}', {(int)tipoBloco}, '{campoAntigo}', '{campoNovo}', NOW(), 1)";
