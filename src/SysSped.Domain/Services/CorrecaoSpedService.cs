...
namespace SysSped.Domain.Services
{
    public class CorrecaoSpedService //: ICorrecaoSpedService
    {
        ...
        public Sped TratarSped(Sped sped)
        {
            ...
            foreach (var bloco100 in sped.BlocosC100)
            {
                ...
                foreach (var bloco170 in bloco100.BlocosC170)
                {
                    ...
                    {
                        var temEan = !string.IsNullOrEmpty(bloco170.COD_NAT);
                        ...
                        {
                            AplicaRegraProdNaoEncontrado(bloco170, tipoNota);
                        }
                    }
                }
        ...
        {
            if (bloco170Arquivo.CST_PIS == "50" && tipoNota == EnumTipoNota.Entrada)
            {
            ...
            else if (bloco170Arquivo.CST_COFINS == "01" && tipoNota == EnumTipoNota.Saida)
            {
                bloco170Arquivo.Tratado = true;
                bloco170Arquivo.CST_PIS_TRATADO = "50";
                bloco170Arquivo.ALIQ_PIS = "1,65";
            ...
            if (bloco170Arquivo.CST_COFINS == "50" && tipoNota == EnumTipoNota.Entrada)
            {
                bloco170Arquivo.Tratado = true;
